...
class Subscription(Base, TimestampMixin):
    ...
    plan_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("public.subscription_plans.plan_id", ondelete="RESTRICT"), nullable=False)
...
class License(Base, TimestampMixin):
    ...
    __table_args__ = {'extend_existing': True}
    id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    subscription_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("public.subscriptions.id", ondelete="CASCADE"), nullable=False)
